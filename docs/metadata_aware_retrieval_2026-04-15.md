...
- 是否已经做过时间过滤
- 当前结果是否已经按时间从早到晚或从晚到早排序
- 如果是 `metadata_first`，说明这是基于时间元数据筛选与排序的结果

这样像：

- `我发的第一条memo是什么`

这类问题，回答模型就不会再因为正文里没有显式写“这是第一条”而误判成“没有相关记录”。

另外，回答链路现在还增加了一个更硬的保护：

- 如果检索结果为空
- 就直接返回固定答复
- 不再进入最终生成模型

这样可以避免出现“明明没有召回到任何结果，但回答模型仍然根据问题胡编内容”的情况。
